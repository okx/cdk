...
import (
	...
	aggTypes "github.com/0xPolygon/cdk/aggregator/ethmantypes"
...
func NewServer(cfg *config.Config, ctx context.Context) *Server {
	...
	srv.ethCfg = ethConfig.Config{
		URL: cfg.L1.RPC,
		EthermanConfig: zkEthman.Config{
			URL: cfg.L1.RPC, // New ethman client "dials" this addr
		},
...
func (s *Server) signSeq(requestData Request) (string, error) {
	...
	l1InfoTreeLeafCount := seqData.L1InfoTreeLeafCount
	expectedInputHash := seqData.ExpectedFinalAccInputHash
	...
	_, data, err := s.ethClient.BuildMockSequenceBatchesTxData(
		s.seqAddress,
		validiumBatchData,
...
func (s *Server) signAgg(requestData Request) (string, error) {
	...
	var inputs = &aggTypes.FinalProofInputs{
